    ...
    try {
      ...
      if (upgradeHeader === "websocket") {
        ...
        if (url.pathname.length == 3 || url.pathname.match(",")) {
          ...
          return await websocketHandler(request, prxIP, proxyType);
        } else if (prxMatch) {
          prxIP = prxMatch[1];
          return await websocketHandler(request, prxIP, proxyType);
        }
        ...
        // No proxy specified
        return await websocketHandler(request, "", proxyType);
      }
...
async function websocketHandler(request, prxIP, proxyType) {
...
// ... protocolSniffer, readStreamHeader, etc ...
// (These are unchanged, but I need to include them in the full file content)
...
async function socks5Connect(proxySocket, targetAddress, targetPort) {
  // 1. Greet
  const writer = proxySocket.writable.getWriter();
  await writer.write(new Uint8Array([0x05, 0x01, 0x00])); // SOCKS5, 1 Method, No Auth
  writer.releaseLock();

  const reader = proxySocket.readable.getReader();
  const { value: greetResponse } = await reader.read();
  reader.releaseLock();

  if (!greetResponse || greetResponse[0] !== 0x05 || greetResponse[1] !== 0x00) {
     throw new Error("SOCKS5 greeting failed");
  }

  // 2. Connect
  const writer2 = proxySocket.writable.getWriter();
  const portBuffer = new Uint8Array(2);
  new DataView(portBuffer.buffer).setUint16(0, targetPort, false); // Big Endian

  let addressType;
  let addressBuffer;

  if (targetAddress.match(/^(\d{1,3}\.){3}\d{1,3}$/)) {
      addressType = 0x01; // IPv4
      addressBuffer = new Uint8Array(targetAddress.split('.').map(Number));
  } else {
      addressType = 0x03; // Domain
      addressBuffer = new Uint8Array([targetAddress.length, ...new TextEncoder().encode(targetAddress)]);
  }

  const request = new Uint8Array([0x05, 0x01, 0x00, addressType, ...addressBuffer, ...portBuffer]);
  await writer2.write(request);
  writer2.releaseLock();

  const reader2 = proxySocket.readable.getReader();
  const { value: connectResponse } = await reader2.read();
  reader2.releaseLock();

  if (!connectResponse || connectResponse[0] !== 0x05 || connectResponse[1] !== 0x00) {
...
async function httpProxyConnect(proxySocket, targetAddress, targetPort) {
    const writer = proxySocket.writable.getWriter();
    const request = `CONNECT ${targetAddress}:${targetPort} HTTP/1.1\r\nHost: ${targetAddress}:${targetPort}\r\n\r\n`;
    ...
    if (!response.includes("200 Connection Established") && !response.includes("200 OK")) {
         throw new Error("HTTP Proxy connection failed: " + response);
    }
...
async function handleTCPOutBound(
  ...
  async function connectAndWrite(address, port) {
    if (prxIP && proxyType) {
        const proxyParts = prxIP.split(/[:=-]/);
        const proxyAddress = proxyParts[0];
        const proxyPort = parseInt(proxyParts[1]);

        log(`connecting to proxy ${proxyAddress}:${proxyPort}`);
        ...
        try {
            if (proxyType === "http") {
                 await httpProxyConnect(tcpSocket, address, port);
            } else {
                 await socks5Connect(tcpSocket, address, port);
            }
            log(`proxy handshake success`);
        } catch (e) {
             console.log("Proxy handshake failed", e);
  ...
  async function retry() {
    if (prxIP && !proxyType) {
        const proxyParts = prxIP.split(/[:=-]/);
        const proxyAddress = proxyParts[0] || addressRemote;
        const proxyPort = parseInt(proxyParts[1]) || portRemote;

        log(`retry via relay ${proxyAddress}:${proxyPort}`);
        const tcpSocket = connect({
            hostname: proxyAddress,
            port: proxyPort,
        });
